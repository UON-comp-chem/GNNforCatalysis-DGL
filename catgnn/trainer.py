...
"""
Shameless hacked from https://github.com/atomistic-machine-learning/schnetpack/blob/master/src/schnetpack/train/trainer.py
...
class Trainer:
   ...
   """
    ...
    def store_checkpoint(self):
        ...
        torch.save(self.state_dict, chkpt, pickle_module=dill)
    ...
    def restore_checkpoint(self, epoch=None):
        ...
        self.state_dict = torch.load(chkpt, pickle_module=dill)
    ...
    def train(self, device, n_epochs=sys.maxsize):
        ...
        try:
            for _ in range(n_epochs):
                ...
                for idx, train_batch in enumerate(train_iter):
                    ...
                    train_batch.graph = train_batch.graph.to(device)
                    train_batch.label = train_batch.label.to(device)
                    result = self._model(train_batch.graph)
                    loss = self.loss_fn(train_batch.label, result)
                    loss.backward()
                    self.optimizer.step()
                    self.step += 1
                ...
                if self.epoch % self.validation_interval == 0 or self._stop:
                    ...
                    for val_batch in self.validation_loader:
                        # append batch_size
                        vsize = len(list(val_batch.label))
                        n_val += vsize
                        ...
                        # move input to gpu, if needed
                        val_batch.graph = val_batch.graph.to(device)
                        val_batch.label = val_batch.label.to(device)
                        #val_batch = {k: v.to(device) for k, v in val_batch.items()}

                        val_result = self._model(val_batch.graph)
                        val_batch_loss = (
                            self.loss_fn(val_batch.label, val_result).data.cpu().numpy()
                        )
