#!/usr/bin/env python
# coding: utf-8
# Author 'John Kitchin', 'Kevin Tran'
# License GNU Lesser General Public License v3.0

"""
Shameless hacked from https://github.com/ulissigroup/GASpy/blob/master/gaspy/mongo.py
Credit goes to GASpy developers
"""
...
def __decode(jsondict):
    """
    A help function to solve the problem that ase 3.19 json.loads(encode(ndarray))
    give a {'__ndarray__': [[3], 'float64', [1.0, 2.0, 3.0]]}, thus the previous
    make atoms from doc failed
    
    """
    assert '__ndarray__' in jsondict.keys()
    jsonstr = jsondict['__ndarray__']
    return np.array(jsonstr[-1]).reshape(jsonstr[0])
...
def make_atoms_from_doc(doc):
    ...
    '''
    if 'array' in doc['atoms']['cell'].keys():
        if isinstance(doc['atoms']['cell']['array'], dict):
            cell = np.array(doc['atoms']['cell']['array']['__ndarray__'][-1]).reshape(3, 3)
        else:
    ...
    try:
        ...
        try:
            atoms = Atoms([Atom(atom['symbol'],
                            __decode(atom['position']),
                            tag=atom['tag'],
                            momentum=__decode(atom['momentum']),
                            magmom=atom['magmom'],
                            charge=atom['charge'])
                       for atom in doc['atoms']['atoms']],
                      cell=cell,
                      pbc=__decode(doc['atoms']['cell']['pbc']),
                      info=doc['atoms']['info'],
                      constraint=[dict2constraint(constraint_dict)
                                  for constraint_dict in doc['atoms']['constraints']])
        except:
            atoms = Atoms([Atom(atom['symbol'],
                       ...
                       for atom in doc['atoms']['atoms']],
                      cell=cell,
                      pbc=doc['atoms']['cell']['pbc'],
                      info=doc['atoms']['info'],
                      constraint=[dict2constraint(constraint_dict)
                                  for constraint_dict in doc['atoms']['constraints']])
    if 'results' in doc.keys():
        results = doc['results']
